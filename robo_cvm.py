"""
============================================================
ROBÔ CVM v2.0 — Catálogo de Fundos Caixa
============================================================
...
O que faz:
  1. Descobre fundos Caixa automaticamente via cadastro CVM
     OU lê uma lista de CNPJs de um arquivo (JSON/CSV/TXT)
  2. Baixa cotas diárias com cache local (não rebaixa o que já tem)
  3. Calcula: variação dia, acum. mês, acum. ano, acum. 12M
  4. Retry automático em cada download
...
Uso:
  ...
  python robo_cvm.py --lista fundos.json     # usa lista específica de CNPJs
  python robo_cvm.py --sem-cache             # força redownload de tudo
  python robo_cvm.py --apenas-abertos        # filtra só fundos abertos para captação
  python robo_cvm.py --saida minha_pasta     # define pasta de saída
============================================================
"""
...
CLASSE_MAP = {
    ...
    "Fundo Mútuo de Privatização - FGTS": "PRIVATIZACAO",
...
# ══════════════════════════════════════════════════════════
def parse_args():
    p = argparse.ArgumentParser(description="Robô CVM — Catálogo de Fundos Caixa v2.0")
    ...
    p.add_argument("--sem-cache",     action="store_true", dest="sem_cache",
                   help="Força redownload de todos os arquivos (ignora cache)")
    p.add_argument("--apenas-abertos", action="store_true", dest="apenas_abertos",
                   help="Inclui só fundos abertos para captação (quando a fonte é JSON Caixa)")
    p.add_argument("--saida",         default=".", dest="saida",
                   help="Pasta de saída para os arquivos gerados (default: pasta atual)")
...
def norm_cnpj(c: str) -> str:
    """Remove pontuação e retorna 14 dígitos com zero-padding."""
...
def fmt_br(v, decimais: int = 2) -> str:
    """Float → string no padrão brasileiro ('1.234,56')."""
...
def baixar_bytes(url: str, label: str = "", sem_cache: bool = False) -> bytes | None:
    ...
    for tentativa in range(MAX_RETRY):
        try:
            log(f"⬇️  Baixando: {label or url}")
            r = requests.get(url, timeout=TIMEOUT, headers={"User-Agent": "Mozilla/5.0"})
            r.raise_for_status()
            os.makedirs(CACHE_DIR, exist_ok=True)
            with open(cache, "wb") as f:
                f.write(r.content)
            log(f"✅  Salvo ({len(r.content)/1e6:.1f} MB)")
            return r.content
        except Exception as e:
            if tentativa == MAX_RETRY - 1:
                log(f"❌  Falhou após {MAX_RETRY} tentativas: {e}")
                return None
            log(f"⚠️   Tentativa {tentativa+1} falhou, retry em 3s...")
            time.sleep(3)
    return None

def baixar_csv_bytes(conteudo: bytes) -> pd.DataFrame:
    """Lê um CSV a partir de bytes (latin1 ou utf-8)."""
...
def baixar_zip_inf(ano: int, mes: int, sem_cache: bool = False) -> pd.DataFrame:
    """Baixa informe diário mensal em ZIP e retorna DataFrame."""
    yyyymm = f"{ano}{mes:02d}"
    url = URL_INF_MES.format(yyyymm=yyyymm)
    label = f"informe diário {yyyymm}"
    dados = baixar_bytes(url, label, sem_cache)
    if dados is None:
        return pd.DataFrame()
    try:
        ...
        log(f"❌  Erro ao abrir ZIP {yyyymm}: {e}")
        return pd.DataFrame()
...
# ══════════════════════════════════════════════════════════
def _autodetectar_lista() -> str | None:
    ...
    for ext in ("*.json", "*.csv", "*.txt"):
        ...
        if arquivos:
            log(f"📂 Arquivo detectado automaticamente: {arquivos[0]}")
            return arquivos[0]
    return None

def ler_lista_cnpjs(caminho: str) -> tuple[list[str], pd.DataFrame | None]:
    """
    Lê CNPJs de: JSON (formato Caixa), CSV ou TXT.
    Retorna (lista_cnpjs, df_extra_ou_None).
    df_extra contém colunas extras do JSON (performance, taxa, etc.)
    """
    ...
    if ext == ".json":
        ...
        if col is None:
            raise ValueError("Coluna CNPJ não encontrada no JSON.")
        df["_CNPJ_NORM"] = df[col].apply(norm_cnpj)
        log(f"   ✅ {len(df)} fundos no JSON (coluna '{col}')")
...
# CADASTRO CVM
# ══════════════════════════════════════════════════════════
def carregar_cadastro(sem_cache: bool = False, filtro_gestor: str | None = None) -> pd.DataFrame:
    """
    ...
    if filtro_gestor:
        ...
        df = df[mask].copy()
        log(f"   ✅ {len(df)} fundos com gestor/admin contendo '{filtro_gestor}'")
        return df

    # Padrão: Caixa em funcionamento
    mask = (
    ...
    df = df[mask].copy()
    log(f"   ✅ {len(df)} fundos Caixa em funcionamento no cadastro CVM")
    return df
...
def preparar_inf(df_raw: pd.DataFrame, cnpjs: set) -> pd.DataFrame:
    ...
    # Detecta coluna CNPJ (varia entre versões do informe)
...
def calcular_indicadores(
    ...
    base = {
        ...
        "PL (milhoes R$)":        "",
        "PL Medio 30d (R$ mi)":   "",
        # Extras do JSON Caixa (preenchidos se disponíveis)
...
def extrair(args) -> pd.DataFrame:
    ...
    print(f"\n{'='*60}")
    print(f"  ROBÔ CVM v2.0 — {hoje.strftime('%d/%m/%Y')}")
    print(f"{'='*60}")
    ...
    if caminho_lista:
        ...
        cnpjs_set   = None  # será preenchido pelo cadastro
        cnpjs_lista = None
    ...
    # Se não tem lista externa, usa todos os CNPJs do cadastro
    ...
    # Adiciona CNPJs da lista que NÃO estão no cadastro (fundos novos ou externos)
    cnpjs_nao_cadastro = cnpjs_set - set(df_cad_filtrado["_CNPJ_NORM"])
    if cnpjs_nao_cadastro:
        log(f"   ⚠️  {len(cnpjs_nao_cadastro)} CNPJs da lista não encontrados no cadastro CVM")
    ...
    # ── Downloads de cotas ────────────────────────────────
    log(f"\n[2/4] Cotas {hoje.year}{hoje.month:02d} (atual) + {ano_ant}{mes_ant:02d} (mês anterior)")
    ...
    for cnpj in cnpjs_set:
        ...
        if cnpjs_json is not None and cnpj in cnpjs_json.index:
            ...
            extras = {
                ...
                "aberto":      "Sim" if row_j.get("ic_aberto_captacao") else "Não",
        ...
        if args.apenas_abertos and extras.get("aberto") == "Não":
            continue
    ...
    log(f"   ✅ {len(df_out)} fundos processados")
    return df_out
...
<head>
<meta charset="UTF-8">
<title>Catálogo Fundos Caixa — CVM</title>
<style>
...
th:hover{{color:#fff}}
th::after{{content:" ⇅";opacity:.35;font-size:10px}}
...
<body>
<h1>Catálogo de Fundos Caixa — CVM</h1>
<p class="sub">Dados: dados.cvm.gov.br &nbsp;·&nbsp; Gerado em {hoje} &nbsp;·&nbsp; github.com/eltonprivatebanker/catalogo-fundos-cvm</p>
...
<div class="box">
  <div class="toolbar">
    ...
    <button class="btn" onclick="mudarPag(-1)">← Anterior</button>
    <span class="pag" id="pagInfo"></span>
    <button class="btn" onclick="mudarPag(1)">Próxima →</button>
  </div>
  <table id="tbl">
...
<div class="footer">
  Fonte: CVM Dados Abertos · Cálculo por variação de cota · Robô CVM v2.0
</div>
...
function paginar() {{
  ...
  rows.forEach(function(r) {{ r.style.display = set.has(r) ? "" : "none"; }});
  document.getElementById("pagInfo").textContent =
    "Pág " + (pag+1) + "/" + totalPag + " · " + total.toLocaleString("pt-BR") + " fundos";
}}
...
function sortTable(th) {{
  ...
  rows.sort(function(a,b) {{
    var va = a.cells[idx]?.innerText.trim() || "";
    var vb = b.cells[idx]?.innerText.trim() || "";
...
def salvar(df: pd.DataFrame, pasta: str):
    ...
    # Remove CNPJ das colunas de saída (é interno)
    ...
    try:
        ...
        log("⚠️  openpyxl não instalado. pip install openpyxl")
