...
def limpar_aba(nome: str) -> str:
    r = re.sub(r'[:\\/*?\[\]]', "", str(nome)).strip()[:31]
    return r if r else "Outros"
...
def carregar_cadastro(sem_cache=False, filtro_gestor=None):
    """Baixa cadastro CVM e retorna fundos da Caixa (ou outro gestor)."""
    ...
    log(f"   Total: {len(df)} fundos | Colunas: {list(df.columns[:8])}...")

    # Normaliza CNPJ
    col_cnpj = next((c for c in df.columns if "cnpj" in c.lower() and "fundo" in c.lower()), None)
    if col_cnpj is None:
        log("[ERRO] Coluna CNPJ nao encontrada!")
        return pd.DataFrame()
    df["_CNPJ_NORM"] = df[col_cnpj].apply(norm_cnpj)

    # Garante coluna DENOM_SOCIAL padronizada
    col_nome = next((c for c in df.columns if any(x in c.lower() for x in ("denom","razao","nome"))), None)
    if col_nome and col_nome != "DENOM_SOCIAL":
        df["DENOM_SOCIAL"] = df[col_nome]

    # Garante coluna CLASSE padronizada
    col_cls = next((c for c in df.columns if "classe" in c.lower()), None)
    if col_cls and col_cls != "CLASSE":
        df["CLASSE"] = df[col_cls]

    # Garante coluna DT_INI_ATIV
    col_dt = next((c for c in df.columns if "dt_ini_ativ" in c.lower() or "dt_reg" in c.lower()), None)
    if col_dt and col_dt != "DT_INI_ATIV":
        df["DT_INI_ATIV"] = df[col_dt]

    # Busca por gestor/admin
    termo = (filtro_gestor or "CAIXA").upper()
    mask  = pd.Series([False] * len(df), index=df.index)

    # Busca em todas as colunas (pandas 3.0 usa StringDtype, nao object)
    for col in df.columns:
        if col in ("_CNPJ_NORM",):
            continue
        try:
            hits = df[col].astype(str).str.upper().str.contains(termo, regex=False)
            if hits.sum() > 0:
                mask |= hits
                log(f"   '{termo}' em {col}: {hits.sum()} fundos")
        except Exception:
            pass
    ...
    # Log situacoes encontradas
    if "SIT" in df_out.columns and not df_out.empty:
        sit_dist = df_out["SIT"].value_counts().to_dict()
        log(f"   Situacoes: {sit_dist}")
